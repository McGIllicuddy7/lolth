...
# 66 "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include/_stdlib.h" 2 3 4
...
# 68 "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include/_stdlib.h" 2 3 4
...
# 59 "./async.h"
void lolth_spawn_task(AsyncTask task);
...
# 3 "main.c" 2
typedef struct testArgs {} testArgs;typedef struct testVars {int i;} testVars;typedef struct testFuture{int returned; testArgs ag; testVars vs;long state;Poll(*poll)(struct testFuture*f, testArgs * a, testVars * vs);}testFuture;Poll testPoll(testFuture* fut, testArgs*a, testVars*v){ (void)a; (void)v; switch(fut->state){ case -1: return PollFinished; case 0: default: { for(v->i=0; v->i<10; v->i++){ printf("%d\n",v->i); fut->state = 7; return PollWait;case 7:(void)(fut);; } return PollFinished; } }}Poll testInterfacePoll(void *ptr){ testFuture* fut = (testFuture*)ptr; return (fut->poll)(fut, &fut->ag, &fut->vs);}testFuture test_call(testArgs ag){ testFuture ret; __builtin___memset_chk (&ret, 0, sizeof(ret), __builtin_object_size (&ret, 0)); ret.ag = ag; ret.state =0; ret.poll = testPoll; return ret;}AsyncTask testvirtual(testArgs ag) { testFuture int = test_call(ag); testFuture * ptr = (testFuture*)malloc(sizeof(int)); *ptr = int; AsyncTask tmp; tmp.poll = testInterfacePoll; tmp.data = ptr; return tmp;}
# 12 "main.c"
typedef struct test_spawnArgs {} test_spawnArgs;typedef struct test_spawnVars {int i;testFuture futs[10]; _Bool finished;} test_spawnVars;typedef struct test_spawnFuture{int returned; test_spawnArgs ag; test_spawnVars vs;long state;Poll(*poll)(struct test_spawnFuture*f, test_spawnArgs * a, test_spawnVars * vs);}test_spawnFuture;Poll test_spawnPoll(test_spawnFuture* fut, test_spawnArgs*a, test_spawnVars*v){ (void)a; (void)v; switch(fut->state){ case -1: return PollFinished; case 0: default: { for(v->i =0; v->i<10; v->i++){ v->futs[v->i] = test_call((testArgs){}); } do { v->finished=1; for(v->i =0; v->i<10; v->i++){ printf("%d:", v->i); if(((v->futs[v->i]).poll(&v->futs[v->i], &v->futs[v->i].ag, &v->futs[v->i].vs) == PollWait)){ v->finished = 0; } } }while(!v->finished); return PollFinished; } }}Poll test_spawnInterfacePoll(void *ptr){ test_spawnFuture* fut = (test_spawnFuture*)ptr; return (fut->poll)(fut, &fut->ag, &fut->vs);}test_spawnFuture test_spawn_call(test_spawnArgs ag){ test_spawnFuture ret; __builtin___memset_chk (&ret, 0, sizeof(ret), __builtin_object_size (&ret, 0)); ret.ag = ag; ret.state =0; ret.poll = test_spawnPoll; return ret;}AsyncTask test_spawnvirtual(test_spawnArgs ag) { test_spawnFuture int = test_spawn_call(ag); test_spawnFuture * ptr = (test_spawnFuture*)malloc(sizeof(int)); *ptr = int; AsyncTask tmp; tmp.poll = test_spawnInterfacePoll; tmp.data = ptr; return tmp;}
# 30 "main.c"
int main(){
 test_spawnFuture t= test_spawn_call((test_spawnArgs){});
 while(((t).poll(&t, &t.ag, &t.vs) == PollWait)){
  printf("testing 123\n");
 }
 return 0;
}
